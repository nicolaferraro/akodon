package io.akodon.maven.plugin;
...
public class RepackageMojo extends AbstractMojo {
    ...
    @Parameter(defaultValue = ".akodon.original")
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        File akodonMaster = resolve("akodon-master");
        File akodonClient = resolve("akodon-client");

        Map<String, DataSource> entries = new TreeMap<>();
        entries.put(AkodonConstants.ZIP_MASTER_JAR_PATH, new FileDataSource(akodonMaster));
        entries.put(AkodonConstants.ZIP_APPLICATION_JAR_PATH, new FileDataSource(backup));
        for(File appProp : getApplicationPropertiesFiles()) {
            entries.put("/" + appProp.getName(), new FileDataSource(appProp));
        }

        repackage(akodonClient, artifact, entries);
    }
